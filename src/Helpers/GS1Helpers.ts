        ...
        if (!isGS) {
            ...
            if(!fixedLengthAI) {
                ...
                if (startPos + gap >= codeWorking.length) {
                    ...
                    dynamicAI = convertDItoAI(dynamicAI);
...
export function convertDItoAI(dynamicAI: string) {
  let converted: string = dynamicAI;
  if (converted.indexOf('9N') === 0 || converted.indexOf('8P') === 0) {
    return '01' + converted.substr(2);
  }
  if (converted.indexOf('S') === 0) {
    return '21' + converted.substr(1);
  }
  if (converted.indexOf('1T') === 0) {
    return '10' + converted.substr(2);
  }
  if (converted.indexOf('D') === 0) {
    return '17' + converted.substr(1);
  }
  return converted;
}
